 ...
 * The latter nonlinear_hf model incorporates an improved *
...
extern real Omega_m_hf, Gamma_hf, sigma_8_hf, n_spec_hf, z0_hf, beta_p_hf;
real s1glob, s2glob;
...
void set_cosmological_parameters_to_default()
{
   Omega_m_hf   = Omega_m;
   Omega_v_hf   = Omega_v;
   Gamma_hf     = Omega_m * H0 * exp(-1.0 * Omega_b - sqrt(2.0 * H0) * Omega_b / Omega_m);
   sigma_8_hf   = sigma_8;
   n_spec_hf    = ns;
...
void dump_param(FILE *F)
{
   if (!F) F = stderr;
   fprintf(F,
	   "#Om Ov G s8 n_s beta_p_hf z0_hf nonlinear_hf = %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d\n",
	   Omega_m_hf, Omega_v_hf, Gamma_hf, sigma_8_hf, n_spec_hf, beta_p_hf, z0_hf, nonlinear_hf);
}
...
void polint(real xa[], real ya[], int n, real x, real *y, real *dy)
{
	...
	*y=ya[ns_hf--];
	for (m=1;m<n;m++) {
		...
		*y += (*dy=(2*ns_hf < (n-m) ? c[ns_hf+1] : d[ns_hf--]));
	}
	free_vector(d,1,n);
	free_vector(c,1,n);
}
...
real da_dtau(real a)
{
   ...
   det = 1. + Omega_m_hf*(1./a - 1.) + Omega_v_hf*(DSQR(a) - 1.);
   res = sqrt(det);
   return res;
}
...
real D_plus(real a)
{
   ...
   if (fabs(Omega_m_hf-OMEGA_M)>epsilon || fabs(Omega_v_hf-OMEGA_V)>epsilon) {
...
void Omega_a(real a, real *omega_m, real *omega_v)
{
   real f, a3;
   a3 = a*a*a;
   f = a + Omega_m_hf*(1.-a) + Omega_v_hf*(a3-a);
   *omega_m = Omega_m_hf/f;
   *omega_v = Omega_v_hf*a3/f;
}
...
real Tsqr(real k)
{
   ...
   return pow(k, n_spec_hf)*DSQR(f1)/sqrt(f2);
}
...
real sigma_8_sqr()
{
   ...
   if (fabs(N_SPEC-n_spec_hf)>epsilon || fabs(GAMMA-Gamma_hf)>epsilon) {
...
real G(real a)
{
   return 1.5*Omega_m_hf/a*g_source(a);
}
...
real P_L(real a, real k)
{
   ...
   t = Tsqr(k);
   s = sigma_8_sqr();
   return DSQR(sigma_8_hf)*DSQR(d)*t/s;
}
...
#define N_kk (500)
   ...
   if (GAMMA!=Gamma_hf || SIGMA!=sigma_8_hf || NSPEC!=n_spec_hf) {

      if (GAMMA!=Gamma_hf) {
	 keff = 0.172 + 0.011*DSQR(log(Gamma_hf/0.36));
	 q8   = 1e-20 + keff/Gamma_hf;
	 ...
	 table[j] = (3+n_spec_hf)*log(q/q8) + 2*log(sigma_8_hf*tk/tk8);
   ...
   if (kk<logkmin || kk>logkmax) {
      q  = 1e-20 + k/Gamma_hf;
      tk = 1/pow(1+pow(6.4*q+pow(3.0*q,1.5)+(1.7*q)*(1.7*q),1.13),(1/1.13));
      res = sigma_8_hf*sigma_8_hf*pow(q/q8,3+n_spec_hf)*tk*tk/tk8/tk8;
   } else {
...
real Delta_L_BE(real k)
{
   ...
   keff=0.172+0.011*log(Gamma_hf/0.36)*log(Gamma_hf/0.36);
   q=1e-20+k/Gamma_hf;
   q8=1e-20+keff/Gamma_hf;
   tk=1/pow(1+pow(6.4*q+pow(3.0*q,1.5)+(1.7*q)*(1.7*q),1.13),(1/1.13));
   tk8=1/pow(1+pow(6.4*q8+pow(3.0*q8,1.5)+(1.7*q8)*(1.7*q8),1.13),(1/1.13));
 
   return sigma_8_hf*sigma_8_hf*pow(q/q8,3+n_spec_hf)*tk*tk/tk8/tk8;
}
...
/* slope in the highly nonlinear_hf regime, c.f. Smith et al (2002) eq. (61) */
...
real P_NL(real a, real k_NL)
{
   ...
   if (OMEGA_M != Omega_m_hf || OMEGA_V != Omega_v_hf || N_SPEC != n_spec_hf ||
       GAMMA != Gamma_hf || SIGMA_8 != sigma_8_hf || NONLINEAR != nonlinear_hf) {
       ...
       */
      if (nonlinear_hf==0) upper = n_spec_hf-4.0;
      else if (nonlinear_hf==1) upper = -2.5;
	 ...
	 if(nonlinear_hf==2) {           /* Smith et al. (2002) */
	    amp = aa*g(aa)/g(1.0);
	    Omega_a(aa, &omm, &omv);
	    golinear = 0;
	 ...
	 iterstart:
	    ...
	    if (iter>=itermax) {
		  ...
		  golinear = 1;
		  upper = table_slope[i] = n_spec_hf-4.0;
		  goto after_wint;
	       } else {
		  goto iterstart;
	       }
	 ...
	 spline(table_k-1, table_P-1, N_k, n_spec_hf, upper, y2-1);
	 klog = logkmin;
	 for (j=0; j<N_k; j++, klog += dk) {
      ...
      GAMMA   = Gamma_hf;
      SIGMA_8 = sigma_8_hf;
      NONLINEAR = nonlinear_hf;
      assert(nonlinear_hf==0 || nonlinear_hf==1 || nonlinear_hf==2);
   }
...
real int_for_w(real a)
{
   real asqr, d;
   asqr = DSQR(a);
   d = a*Omega_m_hf + asqr*(1.-Omega_m_hf-Omega_v_hf) + DSQR(asqr)*Omega_v_hf;
...
/* BS01 2.41, with a(z_2) = 0, a(z_1) = z, in units of Hubble radius_from_mass c/H_0 = 2998/h Mpc */
real w(real a)
{
   static real OMEGA_M = -42.;
   static real OMEGA_V = -42.;
   ...
   if (OMEGA_M != Omega_m_hf || OMEGA_V != Omega_v_hf) {
      da = (1.-a_min)/(N_a-1.);
      aa = a_min;
...
/* S98 2.11. Note that a single redshift sheet (beta_p_hf=0) is implemented
 * only in g_source. */
real prob(real z)
{
   static real norm = 0.;
   ...
   if (fabs(beta_p_hf)<epsilon) error("beta_p_hf=0 in prob");

   if (BETA_P != beta_p_hf || Z0 != z0_hf) {
      norm = beta_p_hf/(z0_hf*exp(gammln(3./beta_p_hf)));
      BETA_P = beta_p_hf;
      Z0     = z0_hf;
   }
...
real g_source(real a)
{
   ...
   /* single source redshift at z0_hf, p(w) = delta(w-w0) */
   if (beta_p_hf == 0.0) {
      aa = 1./(1.+z0_hf);
      if (a<=aa) return 0.0;
      wdelta = w(aa);
   ...
   if (OMEGA_M != Omega_m_hf || OMEGA_V != Omega_v_hf || BETA_P != beta_p_hf || Z0 != z0_hf) {
      da = (1.-a_min)/(N_a-1.);
      table[0] = 0.0;
      aa = a_min+da;
...
real int_for_p_2(real a)
{
   ...
   hoverh0 = sqrt(Omega_m_hf/(a*asqr) + (1.-Omega_m_hf-Omega_v_hf)/asqr + Omega_v_hf);
   res = DSQR(g_source(a)/asqr)/hoverh0;
   if (!finite(res)) assert(0);

   if (nonlinear_hf==1 || nonlinear_hf==2) res *= P_NL(a, f);
   else if (nonlinear_hf==0) res *= P_L(a, f);
   else assert(0);
...
real Pkappa(real s)
{
   ...
   if (OMEGA_M != Omega_m_hf || OMEGA_V != Omega_v_hf || N_SPEC != n_spec_hf ||
       GAMMA != Gamma_hf || BETA_P != beta_p_hf || Z0 != z0_hf ||
       NONLINEAR != nonlinear_hf || SIGMA_8 != sigma_8_hf) {
      logsmin = log(s_min);
      logsmax = log(s_max);
	 ...
	 table[i] = log(9./4.*DSQR(Omega_m_hf)*(f1 + f2));
      }
      OMEGA_M =   Omega_m_hf;
      OMEGA_V =   Omega_v_hf;
      N_SPEC  =   n_spec_hf;
