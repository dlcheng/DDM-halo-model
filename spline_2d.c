...
void spline_2d_free(spline_2d * p)
{
  ...
  for(i=0; i<p->nx; i++)
    {
	gsl_spline_free(p->psp[i]);   /* release the all the splines */
	free(p->data_zx[i]);
    }
...
{
  gsl_interp_accel * acc_global = gsl_interp_accel_alloc();
  ...
  for(i=0; i<p->nx; i++)
     temp_data[i] = gsl_spline_eval(p->psp[i], y, acc_global);
  ...
  gsl_interp_accel_free(acc_global);
